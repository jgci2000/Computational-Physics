...
for i = 1:nShooting
    ...
    % calcular a derivada em x = 0
    DLeft = (25/12*psiLeft(NLeft) - 4*psiLeft(NLeft-1) + 3*psiLeft(NLeft-2) - 4/3*psiLeft(NLeft-3) + 1/4*psiLeft(NLeft-4)) / dx;
    % dpsi(0) = 0
    result(i) = DLeft;
...
fprintf('Os valores de energia em n = 0: %f\n', ENum)
fprintf('O valor de E0 na primeira alínea foi 2.001061 .\n')
% Podemos tirar a mesma conclusão da alínea passada.
