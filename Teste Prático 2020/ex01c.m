...
% Para por o potêncial a 0 de -L/2 a L/2 devido ao isolador coaxial, temos
% de encontrar o indice de -L/2 e L/2 para poder-mos discretizar essa
% região do espaço. Assim,
iMenosL = round((-L/2 + L) / h + 1);
iMaisL = round((L/2 + L) / h + 1);

% Agora podemos por os pontos que representam o isolador coaxial com o
% potêncial nulo e defini-los como pontos de "fronteira" na matriz lim.
VOld(iMenosL:iMaisL, iMenosL:iMaisL) = 0;
...
lim(:, end) = 1;
lim(:, 1) = 1;
lim(iMenosL:iMaisL, iMenosL:iMaisL) = 1;
