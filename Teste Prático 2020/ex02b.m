...
% polyfit() para dE
n = 0:4;
dEAn = 3 * lambda / (4 * omega^2) * (2*n.^2 + 2*n + 1);

% Como E = EHarm + dE, dE  = E - EHarm. Assim, podemos fazer este cálculo
% da seguinte forma
dENum = ENum - EHarmAn;
p = polyfit(n, dENum, 2);
dENum = p(1) * n.^2 + p(2) * n + p(3);

plot(n, dEAn, 'or')
hold on
plot(n, dENum, 'ok')
legend('dE Analítico', 'dE Numérico')
% Pelo gráfico podemos verificar que o cálculo analítico difere um bocado
% do resultado exato. Isto é devido a erros de truncatura devido ao método
% de Numerov e de Shooting.
